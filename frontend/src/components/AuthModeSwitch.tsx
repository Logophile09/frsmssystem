...
/**
 * Wraps the auth card content and plays a directional 3D flip based on
 * which side of the pill switch was just clicked (read from router
 * state set by AuthModeSwitch above) — Register flips in over its right
 * edge, Sign in flips back in over its left edge, like turning a page.
 * Falls back to a plain fade-up (the existing 'page-in' animation) on a
 * direct visit/refresh, where there's no direction to infer.
 */
export function AuthFlipTransition({ children }: { children: React.ReactNode }) {
  ...
  const animationClass =
    dir === 'right' ? 'animate-auth-flip-from-right' : dir === 'left' ? 'animate-auth-flip-from-left' : 'animate-page-in';
