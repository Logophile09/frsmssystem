...
import { AlertTriangle, Inbox, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
export default function IncidentsPage() {
  ...
  const filtered = rows.filter((r) => (search ? JSON.stringify(r).toLowerCase().includes(search.toLowerCase()) : true));
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="font-display text-xl font-bold text-navy-900 dark:text-slate-100">Incidents &amp; Dispatch</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">Log emergencies, assign personnel &amp; vehicles, track status.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search…"
              className="w-48 rounded-lg border border-slate-300 bg-white py-1.5 pl-8 pr-3 text-sm text-navy-900 focus:border-leaf-400 focus:outline-none focus:ring-2 focus:ring-leaf-400/20 dark:border-white/10 dark:bg-navy-800 dark:text-slate-100 dark:placeholder:text-slate-500 dark:text-slate-400"
            />
          </div>
          <button
            onClick={openNew}
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-white/10">
            <thead className="bg-slate-50 dark:bg-white/[0.03]">
              <tr>
                ...
                <th className="px-5 py-2.5 text-left font-semibold text-slate-500 dark:text-slate-400">Alert Level</th>
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-white/[0.04]">
                  ...
                  <td className="whitespace-nowrap px-5 py-2.5 text-right">
                    <button onClick={() => openEdit(r)} className="mr-3 inline-flex items-center gap-1 text-leaf-600 hover:underline dark:text-leaf-300">
                      <Pencil size={13} /> Edit
                    </button>
                    <button onClick={() => remove(r)} className="inline-flex items-center gap-1 text-rose-600 hover:underline dark:text-rose-400">
